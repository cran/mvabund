...
#				'windows', 'win.graph', 'x11', 'X11'.")
	...
	if(isGlm) w <- weights(x, type="prior") 
	else w <- weights(x)
...
#        	     yh  <-  yh[ , - which.na.pass] # !!! needs to be checked, maybe this line must be deleted !!!
	...
	color <- col    
    	...
    	if (any(show[2:4])) { 
      	if (df.residual(x)==0 && studentized && any(show[2:3])) 
            	stop("Plot(s) ", c(2,3)[show[2:3]], " cannot be drawn: 
            		standardized residuals cannot be calculated, as there are no degrees of freedom")
        	else {  
            	wr <- na.omit( as.matrix(weighted.residuals(x)) )[,var.subset,drop=FALSE]
            
			# variance <- (t(wr) %*% wr) / df.residual(x)
            
    			######## to check: use only the diagonal of this variance 
    			######## (as in deviance.mlm,  rep(1, nrow(wr)) %*% wr^2)

            	if(isGlm) s <- sqrt(x$dispersion) 
			else {
            	...
            	if (show[4]) {          
                		cook <- if (isGlm) {   # use cooks.distance.manyglm
                    			cooks.distance(x)[, var.subset, drop=FALSE]
                			  } else {     # the non-weighed residuals are used!
                    			cooks.distance(x, sd = s, res = r.orig)
                			  }
            	...
            	if (any(show[c(2:3)])) {
            		...
            		if(studentized) {
                			ylab23 <- if (isGlm)  "Std. deviance resid."   
							...
							hii     <- manylm.influence(x, do.coef = FALSE)$hat
                    			...
                    			stud <- (1- hii)^(-(1/2))
                    			rs <- matrix(nrow=n, ncol= n.vars)
                    		
						for (i in 1:n.vars) {
                        			rs[,i] <- (diag(stud[,i]) %*% r.w[,i, drop=FALSE])/s[i]
						}
                			} else {
                			...
                			ylab23 <- if (isGlm) "Deviance resid."   
					    	    else "Residuals"
                			rs <- r.w
            		}
	...
	if (any(show[c(1, 3)])) 
      	l.fit <- if (isGlm) "Predicted values"
        		   else "Fitted values"
	...
	####################################################
		...
		if (id.n > 0) # for compatibility with R 2.2.1
                    ylim <- ylim + c(-0.08, 0.08) * diff(ylim)
            	do.call( "plot", c(list( t(yh), t(r), xlab = l.fit, ylab = "Residuals", main = main, ylim = ylim, type = "n", asp=asp ), dots))
