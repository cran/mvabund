...
# If none is an mvabund object, x is the response, if it is not a data.frame.
	...
	if( (is.mvabund(y) & ! is.mvabund(x)  ) | is.data.frame(x)) { 
		...
		mvabund.formula <- x <- as.formula(paste(deparse(targs$y,width.cutoff = 500),
							"~",deparse(targs$x,width.cutoff = 500), sep=""))
	...
	# try to remove x (formula) earlier after giving it the name mvabund.formula
		...
		if(missing(mfrow) & missing(mfcol)) {
			...
			mvabund.formula <- x <- as.formula( paste(deparse(targs$x,width.cutoff = 500),"~",
...
cat("\n \tPIPING to 1st MVFACTOR PLOT \n") 
	do.call( "plotMvaFactor", c(list(x=mvabund.object.1, y=expl.data[,xvar.subset, drop=FALSE],transformation="no"), m, dots))
	return(invisible())
}
########### END factor plot #################
...
cat("\n \tPIPING TO 2nd PLOT FORMULA FEATURE \n")
			do.call( "plotFormulafeature", c(list(mvabund.formula[[yj]], data=data,
					ylab=ylabj, xlab=xlabyj, ask=aske, yaxis.ticks=yaxvalues[[yj]],
						yaxis.labs=yaxislabs[[yj]], fg=fg, cex.xaxis=0.9, cex.yaxis=0.9,
							axes=axes , col=col, pch=pch, type=type, las=las, main= main[yj], ylim=ylimVal,
								border=border, xvar.subset=colsj, cex.lab=1.2,
									all.labels= if(mfr) all.labels else TRUE), dots ))
