 ...
 if (! missing(y) ) {
   ...
   # and whether another function needs to be called
        ...
        if (is.mvabund(y) & is.mvabund(x)) {    
            ...
            if(!any(is.na(subset))) mvabund.object.2 <- mvabund.object.2[c(subset),]
...
cat("\n PIPING TO 1st MVFACTOR \n")
        ...
        if(! any(class(x) == "formula")) {
			...
			if(any(!fac)) warning("Only the factor variables ",
                    				paste((1:ncol(expl.data))[fac], collapse =", "),
                    					" of x will be plotted.")
...
cat("\n PIPING TO 1st MVFORMULA \n")
		...
		if(missing(n.vars)) allargs$n.vars <- min(NCOL(y), 12)
			...
			if(!all(ch)) stop("The two mvabund objects 'x' and 'y' seem not to consist
                  				of the same variables in the same order.")
            	} 
...
cat("\n PIPING TO 2nd MVFORMULA \n")            	
		...
		allargs[[1]]     <- NULL
			...
			if(!all(ch))
                  		stop("The two mvabund objects 'x' and 'y' seem not to consist
                  			of the same variables in the same order.")
            	}
cat("\n PIPING TO 3rd MVFORMULA \n")
 ...
 cat("Kicking off BoxPlot sequence \n")
	...
	xlim <- c(-0.1,1.05*max(mvabund.object.1,na.rm=TRUE))
 	...
 	if (type=="bx") {
		...
		# Add a label for the x axis.
       		mtext(xlab,side=1,line=1.5,col=colab, cex=par("cex.lab")*par("cex")*clab )
	...
	cat("\n\nABOUT TO PLOT THE FUNCTION \n\n")
		...
		do.call( "plot", c(list(mvabund.object.1,y.axis,ylab="",xlab="", main=main,
        			ylim=c(0,n.vars+0.5), pch=pch, axes=FALSE,  type=type, xlim=xlim,
        				col=col),dots))
		...
		# Add a label for the x axis.
      		mtext(xlab,side=1,line=1,cex=par("cex.lab")*par("cex")*clab, col=colab )
	...
	if(n.vars < p) {
		...
		message("Only the variables ", paste(colnames(mvabund.object.1), collapse = ", "),
...
#           			pch=3, horiz=TRUE, text.col=text.col[c(2,1)], cex=0.8,bty="n" )
     		...
     		for (l in 1:windows){
    			...
    			if (par("oma")[3] >= 1) mtext(overall.main, outer = TRUE, cex = 1.1* par("cex.main"),
        							col=par("col.main"), line = -0.5)
    		}
 	}
    	###### END Window LOOP #########
