...
if(! (type %in% c("p", "bx", "n"))) 
      stop("The 'type' ", type,	" is not implemented for factors as independent variables.")
...
if (type=="bx")  {
	...
	for(iexpl in 1:pExpl) {
		...
		axis(side=2,at=unique(y.axisiat)*10, labels=c("",mvabnami[1:(length(mvabnami)-1)]),las=las, pos=posx, col=fg, outer=TRUE, cex.axis=0.75)
...
############  Start a scatterplot  #################
	...
	for(iexpl in 1:pExpl) {
		...
		par(mgp=c(1.5,1,0), mar= c(4, 6, 1, 4) + 0.1, oma= c(1,1,1,1) )                
		do.call( "plot", c(list(mvabund.object,y.axisi,ylab="", xlab="", main=main, ylim=c(0,n.vars+0.5), pch=pch, axes=FALSE, type=type, xlim=xlimi, col=col, cex=cex),dots))
		...
		axis( side=2, at=c((n.vars+0.5), n.vars:1,0.5), labels=c("" , substring(mvabund.colnames,first=1, last= max(10, nchar(mvabund.colnames, type="char") )  ),""), las=las,	pos=posx, col=fg, outer=TRUE, cex.axis=0.6)

		# A label for the y axis.
		mtext(ylab,side=2,line=6,cex=par("cex.lab")*par("cex")*clab, col=colab )
...
########## set x-axis
		...
		mtext(xlab,side=1,line=2,cex=par("cex.lab")*par("cex")*clab, col=colab )		
		rec.x = seque[length(seque)] + 1/20*seque[length(seque)] 
		...
		if(legend) {
			...
			par(xpd=NA)
			legend(title=substr(factornames[iexpl],1,10), y =n.vars+0.4,x=1.05*max(seque), legend=leg, horiz=legend.horiz,col=unique(col), cex=0.8, ncol=ncoll, pch=unique(pch), bg=0)
