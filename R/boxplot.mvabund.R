...
################################################################################
boxplot.mvabund <-
function (x, y=NULL, ..., range = 1.5, width = NULL,
    # varwidth = FALSE,
...
allargs <- lapply(allargs, eval, parent.frame())
   ...
   args <- list(x, y)
	...
	# If args have names: namedargs are TRUE, else FALSE.
	 namedargs <- if (!is.null(attributes(dots)$names))
        attributes(dots)$names != ""
  ...
  # Plot x and each of the unnamed dots values (which are not allowed to be a list),
	# by using several calls to 'plot.mvabund'.
		if (!is.null(dotsnotnamed)){
			...
			do.call( "plot.mvabund", c(list(x=x, type="bx"),dotsnamed,allargs) ,
        quote=FALSE)
			if(!missing(y))
				do.call( "plot.mvabund", c(list(x=y, type="bx"),dotsnamed,allargs),
          quote=FALSE)

    	for(i in 1:length(dotsnotnamed)){
				do.call( "plot.mvabund", c(list(x=dotsnotnamed[[i]], type="bx"),
          dotsnamed,allargs), quote=FALSE)
			}
    ...
    # Plot x and an optional y.
			if(!missing(y)) {
				do.call( "plot.mvabund", c(list(x=x, y=y, type="bx"),dotsnamed,allargs),
          quote=FALSE)
			} else {
				do.call( "plot.mvabund", c(list(x=x, type="bx"),dotsnamed,allargs),
          quote=FALSE)
