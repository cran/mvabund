...
function (x, y=NULL, range = 1.5, width = NULL,
    notch = FALSE, outline = TRUE, names=NULL,
    ...
    overall.main="Boxplot", var.subset=NA, transformation="log", scale.lab="s", t.lab="o",
    mfrow=1, mfcol=NULL,
	...
	# by using several calls to 'plot.mvabund'.
		if (!is.null(dotsnotnamed)){
			...
			if(!missing(y) && !is.null(y))
				do.call( "plot.mvabund", c(list(x=y, type="bx"),dotsnamed,allargs),  quote=FALSE)
